...
class ExperimentDatabase:
    ...
    def log_experiment(self, experiment_data: Dict[str, Any]) -> int:
        try:
            ...
            logger.error(f"Ошибка логирования эксперимента: {e}")
            return -1
    ...
    def _update_model_metrics(self, prediction_data: Dict[str, Any]):
        try:
            ...
            logger.error(f"Ошибка обновления метрик модели: {e}")
    ...
    def get_model_comparison(self) -> pd.DataFrame:
        try:
            ...
            logger.error(f"Ошибка получения сравнения моделей: {e}")
            return pd.DataFrame()
    ...
    def clear_predictions_older_than(self, days: int):
        try:
            ...
            logger.error(f"Ошибка удаления старых предсказаний: {e}")
